...
function add_items_menu($name, $args = array(), $location = array()) {
  ...
  if( !empty( $location ) && isset( $location[ 'bool' ] ) ){
    $name = $location[ 'bool' ]; $value = $new_menu_ID;
    $mods = get_theme_mods();
    ...
    if($update === false) {
    	  return new WP_Error( 'invalid_update_option', __( 'The option "theme_mods_' . $theme . '" is not update.', 'item-menu' ) );
    }
  }
}
